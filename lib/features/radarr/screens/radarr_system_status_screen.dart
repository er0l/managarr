...
class RadarrSystemStatusScreen extends ConsumerWidget {
  const RadarrSystemStatusScreen({super.key, required this.instance});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return _Section(
      ...
      trailing: IconButton(
        ...
        onPressed: () => ref.invalidate(radarrHealthProvider(instance)),
      ),
        ...
        data: (status) => Column(
          children: [
            ...
            if (status.runtimeVersion != null)
              _InfoRow(label: 'Runtime', value: status.runtimeVersion!),
            if (status.appData != null)
              _InfoRow(label: 'App Data', value: status.appData!),
            if (status.startupPath != null)
              _InfoRow(label: 'Startup Path', value: status.startupPath!),
          ],
        ),
      ),
    );
